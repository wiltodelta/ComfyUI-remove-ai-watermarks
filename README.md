...
- All four profiles are CUDA-only: `qwen-zimage` (the default), `sdxl-zimage`,
  `chroma-zimage`, and `auto` (chroma-zimage for OpenAI and Microsoft
  provenance, qwen-zimage otherwise). There is no CPU or MPS path, so the
  node has no device widget.
